...
class ListProjects30Fragment : Fragment() {
	companion object {
		const val FRAGMENT_TAG = "ListProjects30Fragment"
	}
	...
	private fun updateListItems() {
		...
		val projects =
			storageHelper.getAllProjects().filter { it.slot == Slot.SLOT_30 }.sortedBy { it.name }
