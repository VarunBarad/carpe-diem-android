...
class ListProjects03Fragment : Fragment() {
	companion object {
		const val FRAGMENT_TAG = "ListProjects03Fragment"
	}
	...
	private fun updateListItems() {
		...
		val projects = storageHelper.getAllProjects().filter { it.slot == Slot.SLOT_03 }.sortedBy { it.name }
